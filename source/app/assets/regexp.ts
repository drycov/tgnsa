...
const createRegExp = (pattern: string, flags: string = "") =>
  new RegExp(pattern, flags);

const createBotnameRegexp = (botname: string) =>
  createRegExp(`\\@${botname}\\s`);
const createSubstringReplacer = (botname: string) => (val: string) =>
  val.trim().replace(createBotnameRegexp(botname), "");

const IpRegexp = createRegExp(
...
// Функции проверки
const createCheckFunction = (regexp: RegExp) => (val: string) =>
  regexp.test(val.trim());
